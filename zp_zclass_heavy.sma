...
public use_skill(id)
{
	if(is_user_alive(id) && zp_get_user_zombie(id) && zp_get_user_zombie_class(id) == g_zheavy && !zp_get_user_nemesis(id))
	{
		if(can_make_trap[id])
		{
			create_trap(id)
			} else {
			client_print(id, print_chat, "[ZP] You can't Make a Trap at this time. Wait for %i seconds.", get_pcvar_num(cvar_cooldown))
		}
	}
}
...
public reset_cooldown(taskid)
{
	...
	if(is_user_alive(id) && zp_get_user_zombie(id) && zp_get_user_zombie_class(id) == g_zheavy)
	{
		...
		{
			can_make_trap[id] = true
			client_print(id, print_chat, "[ZP] Now. You can Make a Trap, Press (G)")
		}
	}	
}
